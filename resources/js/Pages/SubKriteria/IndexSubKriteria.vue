...
const { setKriteria, setSubkritea, moveListItem, submitForm, resetForm } = store;
...
const handleUpdate = (max, idKriteria) => {
...
<template>
  ...
  <AuthenticatedLayout>
    <section class="h-full">
      <IndexCrudTable :list="formattedKriteriaList">
        ...
        <template #table-header>
          <div class="w-6/12 max-md:w-full bg-content p-4 rounded-lg">
            ...
            <SearchInput label="Kriteria" :is-edit="massEdit" @search="searchKriteria" />
          </div>
        </template>
            ...
            <TableCrud :list="kriteria.subkriteria" :max-rank="kriteria.subkriteria.length + 1"
              ...
              <template #sub-table-header>
                <div class="flex flex-col md:flex-row justify-between p-4 text-center">
                  ...
                  <div class="flex flex-col space-y-3 md:flex-row md:space-x-4 md:space-y-0 items-center">
                    <SearchInput label="Sub Kriteria" :is-edit="false"
                      @search="(e) => searchSubKriteria(e, kriteria.id)" />
                    <ButtonGroupTable @on-show-create="handleShowCreate(kriteria.id)" />
                  </div>
                </div>
                <div
                  class="flex justify-between px-2 lg:px-4 lg:mb-4 border-b-2 dark:border-b-gray-700 border-b-gray-300">
                </div>
              </template>
              ...
              <template #tbody-content>
                <tr v-for="(item, index) in kriteria.subkriteria" class="t-row" :id="'kriteria' + index">
                  ...
                  <td class="px-1 py-3 flex justify-center">
                    ...
                    <div :id="item.nama + item.id + 'dropdown'"
                      ...
                      <ul class="py-1" :aria-labelledby="item.nama + item.id">
                        <li>
                          <button type="button" @click="handleShowUpdate(item)"
                            data-modal-target="updateProductModalmain" data-modal-toggle="updateProductModalmain"
