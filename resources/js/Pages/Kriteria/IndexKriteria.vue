      ...
      <IndexCrudTable>
        ...
        <template #table>
          <TableCrud :max-rank="dataList.length + 1" :list="filteredList" :search-query="queryKriteria"
            :class="'lg:mx-4 lg:mb-4 lg:rounded-lg'" @create="handleCreate" @update="handleUpdate"
            @delete="handleDelete">
            <SubKriteriaModal />
            ...
            <template #tbody-content>
              <tr v-for="(item, index) in filteredList" class="t-row" :id="'kriteria' + index">
                ...
                <TData v-if="!massEdit">
                  <a @click="
                    ...
                    setKriteria(item)
                    " data-modal-target="sub-kriteria-modal" data-modal-toggle="sub-kriteria-modal"
                ...
                <td class="px-1 py-3 flex justify-center">
                  ...
                  <div class="move-buttons" v-if="massEdit">
                    ...
                    <SecondaryButton @click="moveListItem(index, 'down')" :disabled="index === kriteriaList.length - 1">
                      <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
