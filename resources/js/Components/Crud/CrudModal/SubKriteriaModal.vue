...
<template>
  <!-- Main modal -->
  <div id="sub-kriteria-modal" tabindex="-1" aria-hidden="true"
    class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-2xl max-h-full">
      <!-- Modal content -->
      <div class="relative bg-gray-300 rounded-lg shadow dark:bg-gray-800">
        <!-- Modal header -->
        <div class="flex items-center justify-between border-b rounded-t dark:border-gray-600 py-2 px-4">
          <h3 class="text-body-normal font-semibold text-gray-900 dark:text-white">
          ...
          <button type="button"
            ...
            data-modal-target="sub-kriteria-modal" data-modal-hide="sub-kriteria-modal">
        ...
        <TableCrud :list="subKriteriaList" :error-message="'Belum Memiliki Sub Kriteria'">
          ...
          <template #tbody-content>
            <tr v-if="subKriteriaList.length > 0" v-for="(item, index) in subKriteriaList" class="t-row"
              :id="'sk' + index">
