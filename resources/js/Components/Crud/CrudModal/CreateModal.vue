<template>
  <div :id="`createProductModal${id}`" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-2xl max-h-full">
      ...
      <div class="relative p-4 bg-primary-50 rounded-lg shadow dark:bg-gray-800 sm:p-5">
        ...
        <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
          ...
          <!-- Close Modal -->
          <button type="button" :data-modal-target="`createProductModal${id}`"
            :data-modal-hide="`createProductModal${id}`"
          ...
          <div class="grid gap-4 mb-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input type="number" name="brand" id="brand"
                class="bg-primary-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                :placeholder="'Masukan angka antara 1 s.d. ' + maxRank" v-model="form.rank.value" />

              <InputError class="mt-2" :message="form.errors['rank.value']" />
            </div>
          </div>

          <!-- Button Tambahkan + hide -->
          <button type="submit"
